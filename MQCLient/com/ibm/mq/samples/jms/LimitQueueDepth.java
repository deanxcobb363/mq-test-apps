...
// consider loop here
      for (int i = 0; i < 100; i++) {
			...
			TextMessage message = context.createTextMessage("\n  Message Received from Queue with Unique Number: " + uniqueNumber);

			producer = context.createProducer();
			producer.send(destination, message);
//			System.out.println("Sent message:\n" + message);
			System.out.println("\n  Message Sent to Queue 4 with Unique Number: " + uniqueNumber);
    	recordSuccess();
			try{
				TimeUnit.SECONDS.sleep(3);
			} catch (InterruptedException interruptEx) {
				recordFailure(interruptEx);
			}
